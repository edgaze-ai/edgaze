-- Signup must not fail if public.profiles already has a row for NEW.id (retries, races, or
-- claim-transfer overlapping with auth). GoTrue surfaces DB failures as "Database error saving new user".
...
DECLARE
  ...
  attempt int := 0;
BEGIN
  IF NEW.raw_app_meta_data @> '{"edgaze_defer_profile_row": true}'::jsonb
     OR NEW.raw_app_meta_data->>'edgaze_defer_profile_row' IN ('true', '1', 't')
     OR NEW.raw_user_meta_data @> '{"edgaze_defer_profile_row": true}'::jsonb
     OR NEW.raw_user_meta_data->>'edgaze_defer_profile_row' IN ('true', '1', 't') THEN
    RETURN NEW;
  END IF;
