-- Admin creator provisioning inserts the profile row via API upsert with the final handle.
-- Skipping the trigger INSERT avoids GoTrue returning only `unexpected_failure` when profile
-- insert violates constraints or depends on env-specific DB state.
...
BEGIN
  IF NEW.raw_app_meta_data @> '{"edgaze_defer_profile_row": true}'::jsonb
     OR NEW.raw_app_meta_data->>'edgaze_defer_profile_row' IN ('true', '1', 't') THEN
    RETURN NEW;
  END IF;
