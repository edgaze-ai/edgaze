-- Admin accounting: unified marketplace transactions and summary stats.
-- Called only via service role from admin API routes (SECURITY DEFINER; no grants to anon/authenticated).

CREATE OR REPLACE FUNCTION public.admin_marketplace_stats()
RETURNS TABLE (
  workflow_sales_total bigint,
  prompt_sales_total bigint,
  paid_workflow_gmv_cents bigint,
  paid_prompt_gmv_cents bigint
)
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $$
  SELECT
    (SELECT count(*)::bigint FROM public.workflow_purchases WHERE stripe_payment_intent_id IS NOT NULL),
    (SELECT count(*)::bigint FROM public.prompt_purchases WHERE stripe_payment_intent_id IS NOT NULL),
    (
      SELECT coalesce(sum(amount_cents), 0)::bigint
      FROM public.workflow_purchases
...
REVOKE ALL ON FUNCTION public.admin_marketplace_stats() FROM public;
REVOKE ALL ON FUNCTION public.admin_marketplace_transactions_page(integer, integer, text, text) FROM public;

GRANT EXECUTE ON FUNCTION public.admin_marketplace_stats() TO service_role;
GRANT EXECUTE ON FUNCTION public.admin_marketplace_transactions_page(integer, integer, text, text) TO service_role;
