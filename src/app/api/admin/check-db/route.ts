...
export async function GET() {
  try {
    ...
    console.log('[Check DB API] User:', user?.id);
    
    // Try to query creator_invites table directly
    const { data: invites, error: invitesError, count } = await supabase
      .from('creator_invites')
      .select('*', { count: 'exact' })
      .order('created_at', { ascending: false })
      .limit(10);

    console.log('[Check DB API] Query result:', { count, invitesLength: invites?.length, error: invitesError });
