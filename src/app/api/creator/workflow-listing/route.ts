...
type Body = {
  ...
  /** True when thumbnail is from the builder auto generator (not a user-uploaded image). */
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data: existing } = await admin
      .from("workflows")
      .select("id,owner_id,published_at,edgaze_code,thumbnail_auto_generated")
      .eq("id", workflowId)
      .maybeSingle();
    ...
    let thumbnailAutoGenerated: boolean | null = null;
    if (typeof body.thumbnailAutoGenerated === "boolean") {
      thumbnailAutoGenerated = body.thumbnailAutoGenerated;
    } else if (existingRow) {
      thumbnailAutoGenerated = existingRow.thumbnail_auto_generated ?? null;
    }
