        ...
        <div className="border-t border-amber-500/20 p-4 space-y-4">
          ...
          <div>
            ...
            <p className="mb-2 text-xs text-white/40">
              Example: From <code className="text-cyan-400">http://localhost:3000/c/ABC123...</code> paste just <code className="text-cyan-400">ABC123...</code>
            </p>
              ...
              {debugData.error ? (
                ...
                <div className="space-y-3">
                  ...
                  <div className="rounded-lg border border-white/[0.08] bg-black/40 p-3">
                    ...
                    <div className="space-y-1 text-xs">
                      <div className="flex justify-between">
                        ...
                        <span className="font-mono text-white">{debugData.debug?.token_length}</span>
                      </div>
                      <div className="flex justify-between">
                    ...
                    {debugData.debug?.lookup_by_hash?.invite && (
                      <div className="space-y-1 text-xs">
                        <div className="flex justify-between">
                          ...
                          <span className="text-white">{debugData.debug.lookup_by_hash.invite.creator_name}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-white/60">Status:</span>
                          <span className={`font-semibold ${
                            debugData.debug.lookup_by_hash.invite.status === 'active' ? 'text-cyan-400' : 'text-amber-400'
                          }`}>
                        ...
                        <div className="flex justify-between">
                          ...
                          <span className={debugData.debug.lookup_by_hash.invite.has_raw_token ? 'text-green-400' : 'text-red-400'}>
                  ...
                  <div className={`rounded-lg border p-3 ${
                    ...
                    {debugData.debug?.lookup_by_raw_token?.invite && (
                      <div className="space-y-1 text-xs">
                        <div className="flex justify-between">
                          <span className="text-white/60">Creator:</span>
                          <span className="text-white">{debugData.debug.lookup_by_raw_token.invite.creator_name}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-white/60">Status:</span>
                          <span className={`font-semibold ${
                            debugData.debug.lookup_by_raw_token.invite.status === 'active' ? 'text-cyan-400' : 'text-amber-400'
                          }`}>
                  ...
                  <div className="rounded-lg border border-white/[0.08] bg-black/40 p-3">
                    ...
                    {!debugData.debug?.recent_invites || debugData.debug.recent_invites.length === 0 ? (
                        ...
                        >
                          <div className="flex justify-between items-center mb-1">
                            ...
                            <div className="flex gap-2 items-center">
                              ...
                              <span className={`text-xs ${inv.status === 'active' ? 'text-cyan-400' : 'text-white/40'}`}>
                                {inv.status}
                              </span>
                            </div>
                          </div>
                          <div className="mt-1 space-y-0.5 text-white/50">
                            ...
                            <div className="flex justify-between">
                              <span>Hash:</span>
                              <span className="font-mono text-[10px]">{inv.token_hash_preview}</span>
                            </div>
                          </div>
                ...
                <div className="space-y-2">
                  <div className="rounded-lg border border-white/[0.08] bg-black/40 p-3">
                    <div className="flex justify-between text-xs">
                      ...
                      <span className={dbCheck.database_check?.table_exists ? 'text-green-400 font-semibold' : 'text-red-400 font-semibold'}>
                        {dbCheck.database_check?.table_exists ? 'Yes ✓' : 'No ✗'}
                      </span>
                    </div>
                    <div className="flex justify-between text-xs mt-1">
                      <span className="text-white/60">Total Invites:</span>
                      <span className="text-white font-semibold">{dbCheck.database_check?.total_invites || 0}</span>
                    </div>
                    {dbCheck.database_check?.query_error && (
                      <div className="mt-2 pt-2 border-t border-white/[0.08]">
                        <p className="text-xs text-red-400">Query Error: {dbCheck.database_check.query_error}</p>
                        <p className="text-xs text-white/40 mt-1">Code: {dbCheck.database_check.query_code}</p>
                      </div>
                    )}
                  </div>
                </div>
              ...
              {allInvites.error ? (
                ...
                <div className="space-y-2">
                  {allInvites.invites?.map((inv: any) => (
                    <div key={inv.id} className="rounded-lg border border-white/[0.08] bg-black/40 p-3">
              ...
              {migrations.error ? (
                ...
                <div className="space-y-3">
                  {migrations.migrations?.map((mig: any, idx: number) => (
                    <div key={idx} className="rounded-lg border border-white/[0.08] bg-black/40 p-3">
                      <div className="flex justify-between items-center mb-2">
