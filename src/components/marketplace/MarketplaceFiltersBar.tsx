...
// Estimate chip width: horizontal padding + charWidth per character
function estChipWidth(label: string, isMobile: boolean): number {
  const px = isMobile ? 20 : 24; // px-2 py-1 / px-2.5 py-1.5
  const charW = isMobile ? 6 : 7;
  return px + label.length * charW;
}

// Compute max topics that fit in given width
function fitTopics(availableWidth: number, topics: string[], gap: number, isMobile: boolean): number {
  const buffer = 4;
  let used = 0;
...
export default function MarketplaceFiltersBar({
  ...
  const [topicDropdownPos, setTopicDropdownPos] = useState<{ top: number; left: number } | null>(null);
  ...
  useEffect(() => {
    const handleClickOutside = (e: MouseEvent) => {
      ...
      if (
        topicTriggerRef.current?.contains(target) ||
        topicDropdownRef.current?.contains(target)
      ) {
        return;
      }
  ...
  // Clean YouTube-style chip styles — no glows or gradient borders
  const pillActive =
    "bg-white/12 text-white border border-white/20";
  const pillInactive =
    "bg-transparent text-white/70 hover:bg-white/5 hover:text-white/90 border border-transparent";

  return (
    <section className="relative z-[70] mb-6">
      <div className="rounded-xl border border-white/[0.08] bg-[#0c0c0f]/95 backdrop-blur-xl shadow-[0_0_0_1px_rgba(255,255,255,0.04),0_4px_20px_rgba(0,0,0,0.35)] overflow-hidden">
        <div className="flex items-center">
        {/* Topics area: fills space, scrolls horizontally when needed */}
        <div className="flex-1 min-w-0 overflow-x-auto overflow-y-hidden scrollbar-hide scroll-smooth" style={{ scrollbarWidth: "none", msOverflowStyle: "none" }}>
          <div className="flex flex-nowrap items-center gap-2 sm:gap-3 min-w-max px-3 py-2 sm:px-4 sm:py-2.5">
            {/* Sort */}
            <div className="flex items-center gap-1.5 shrink-0">
              <Sparkles className="h-3 w-3 sm:h-3.5 sm:w-3.5 text-cyan-400/90 shrink-0" />
              <span className="text-[10px] font-semibold tracking-[0.18em] text-white/50 uppercase hidden sm:inline">
                Sort
              </span>
            </div>
              ...
              {SORT_OPTIONS.map((opt) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      "inline-flex shrink-0 items-center gap-1.5 rounded-lg px-2.5 py-1.5 sm:px-3 sm:py-2 text-xs font-medium transition-all duration-200 border",
                      active ? pillActive : pillInactive
                    )}
                  >
                    <Icon className={cn("h-3 w-3 sm:h-3.5 sm:w-3.5 shrink-0", active ? "text-cyan-300" : "text-white/50")} />
            ...
            {/* Divider */}
            <div className="h-4 w-px bg-white/[0.08] shrink-0 mx-0.5" aria-hidden />
            {/* Topics */}
            <div className="flex items-center gap-1.5 shrink-0 min-w-0">
              <Tag className="h-3 w-3 sm:h-3.5 sm:w-3.5 text-white/45 shrink-0" />
              <span className="text-[10px] font-semibold tracking-[0.15em] text-white/50 uppercase hidden lg:inline shrink-0">
                Topics
              </span>
            </div>
            <div
              ...
              className="flex gap-1 sm:gap-1.5 min-w-max shrink-0"
            >
              {TOPIC_OPTIONS.map((t) => {
                const isAll = t === "All";
                ...
                return (
                  <button
                    ...
                    onClick={() => setTopic(t)}
                    className={cn(
                      "shrink-0 rounded-lg border font-medium transition-all duration-200",
                      "text-[10px] sm:text-xs px-2 py-1 sm:px-2.5 sm:py-1.5",
        ...
        {/* Filters — fixed to the right, never scrolls */}
        <div className="shrink-0 border-l border-white/[0.08] pl-2 pr-3 py-2 sm:pr-4 sm:py-2.5">
          <button
            type="button"
            onClick={() => setCustomOpen((o) => !o)}
            className={cn(
              "inline-flex items-center gap-1.5 rounded-lg px-2.5 py-1.5 sm:px-3 sm:py-2 text-xs font-medium transition-all duration-200 border",
          ...
          >
            ...
            {activeFilterCount > 0 && (
              <span className="flex h-4 min-w-[16px] items-center justify-center rounded-full bg-cyan-400/40 px-1 text-[10px] font-bold text-cyan-100">
                {activeFilterCount}
              </span>
            )}
          </button>
